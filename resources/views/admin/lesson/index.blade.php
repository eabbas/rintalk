@extends('welcome')
@section('title', 'همه درس ها')
@section('content')
    <div class="w-full flex flex-col pb-4">
        <div class="bg-white rounded-lg">
            <h2 class="text-lg font-bold text-gray-800 p-4 text-center">لیست دروس </h2>
                        ...
                        @foreach ($lessons as $lesson)
                            <div class="w-full grid grid-cols-10 items-center divide-x divide-[#f1f1f4]">
                                ...
                                <div class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center col-span-2">
                                    <span class="block w-20 lg:w-full">{{ $lesson->title }}</span>
                                </div>
                                <div class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center col-span-1">
                                    <span class="block w-20 lg:w-full">{{ $lesson->price }}</span>
                                </div>
                                <div class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center col-span-1">
                                    <span class="block w-20 lg:w-full">{{ $lesson->duration }}</span>
                                </div>
                                <div class="col-span-5 flex flex-row items-center justify-center gap-1 flex-wrap">
                                    <a href="{{ route('lesson.single', [$lesson]) }}"
                                        class="w-fit flex items-center justify-center bg-sky-500 hover:bg-sky-600 p-1.5 rounded-sm"
                                        title="مشاهده">
                                    ...
                                    <a href="{{ route('lesson.delete', [$lesson]) }}"
                                    ...
                                    <a href="{{ route('LessonAttachment.create', [$lesson]) }}"
