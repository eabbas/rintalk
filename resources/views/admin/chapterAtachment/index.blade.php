...
@section('content')
        ...
        <span class="block w-24 lg:w-full">عملیات</span>
                    ...
                    <div class="bg-white divide-y divide-[#f1f1f4]">
                        @php
                            $i = 1;
                        @endphp
                       @foreach ($ChapterAtachments as $ChapterAtachment)
    <div class="w-full grid grid-cols-10 items-center divide-x divide-[#f1f1f4]">
        ...
        <div class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center col-span-2">
            <span class="block w-20 lg:w-full">{{ $ChapterAtachment->title }}</span>
        </div>
        <div class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center col-span-2">
            <span class="block w-20 lg:w-full">{{ $ChapterAtachment->price }}</span>
        </div>
        <div class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center col-span-2">
            <span class="block w-20 lg:w-full">{{ $ChapterAtachment->summary }}</span>
        </div>
        <div class="col-span-3 flex flex-row items-center justify-center gap-1">
    <a href="{{ route('chapterAtachment.chapterAtachmentEdit', [$ChapterAtachment->id]) }}"
    ...
    <a href="{{ route('chapterAtachment.delete', [$ChapterAtachment->id]) }}"
