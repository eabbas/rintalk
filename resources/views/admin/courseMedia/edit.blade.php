@extends('welcome')
    ...
    @section('content')
     <h1 class="text-2xl font-bold text-gray-800 text-center mb-5">فرم ویرایش مدیا</h1>
        <form action="{{ route('courseMedia.update') }}" method="post" enctype='multipart/form-data'>
            @csrf
            <input type="hidden" name="id" value="{{ $courseMedia->id }}">
                            ...
                            <div class="w-full flex flex-col gap-3 max-md:flex-col max-md:gap-1">
                                ...
                                <div
                                    ...
                                    <input class="p-4 w-full focus:outline-none text-sm font-bold mr-2" type="text"
                                        name='duration' placeholder="مدت زمان(دقیقه) مدیا راوارد کنید" value="{{$courseMedia->duration}}">
                                </div>
                            </div>
                            <div class="w-full flex flex-col gap-3 max-md:flex-col max-md:gap-1">
                                ...
                                <div
                                    ...
                                    <select name="course_id" class="p-4 w-full focus:outline-none text-sm font-bold mr-2">
                                        @foreach ($courses as $course)
                                            ...
                                            <option value="{{$course->id}}"  @if ($course->id == $courseMedia->course_id ) {{'selected'}} @endif>{{$course->title}}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="w-full flex flex-col gap-3 max-md:flex-col max-md:gap-1">
                                ...
                                <div
                                    ...
                                    <input class="p-4 w-full focus:outline-none text-sm font-bold mr-2" type="number" step="1"
                                        name='order' placeholder="عدد رو وارد کنید" value="{{$courseMedia->order}}">
                                </div>
                            </div>
                            </div> 
                        </div>
