@extends('welcome')
    ...
    @section('content')
     <h1 class="text-2xl font-bold text-gray-800 text-center mb-5">فرم ویرایش مدیا</h1>
        <form action="{{ route('chapterMedia.chapterMediaUpdate') }}" method="post" enctype='multipart/form-data'>
            @csrf
            <input type="hidden" name="id" value="{{ $chapterMedia->id }}">
                        ...
                        <div class="w-full grid grid-cols-1 lg:grid-cols-2 gap-3 my-4">
                            ...
                            <div class="w-full flex flex-col gap-3 max-md:flex-col max-md:gap-1">
                                ...
                                <div
                                    class="rounded-lg focus:border-none focus:outline-none focus:bg-[#F1F1F4] bg-[#F9F9F9] text-[#99A1B7] w-full flex">
                                    <input class="p-4 w-full focus:outline-none text-sm font-bold mr-2" type="text"
                                        name='duration' placeholder="مدت زمان(دقیقه) مدیا راوارد کنید" value="{{$chapterMedia->duration}}">
                                </div>
                            </div>
                            <div class="w-full flex flex-col gap-3 max-md:flex-col max-md:gap-1">
                                ...
                                <div
                                    ...
                                    <input class="p-4 w-full focus:outline-none text-sm font-bold mr-2" type="number" step="1"
                                        name='order' placeholder="عدد رو وارد کنید" value="{{$chapterMedia->order}}">
                                </div>
                            </div>
                            </div> 
                        </div>
