@extends('welcome')
@section('title', 'همه پیوست ها')
@section('content')
    <div class="w-full flex flex-col pb-4">
        <div class="bg-white rounded-lg">
            <h2 class="text-lg font-bold text-gray-800 p-4 text-center">لیست پیوست ها </h2>
              <div class="flex justify-end w-10/12 mx-auto">
                    <a href="{{ route('lesson.lessons') }}"
                        class="px-5 py-1 mb-4 rounded-sm bg-[#eb3254] hover:bg-rose-600 text-white text-xs lg:text-base">لیست دروس</a>
                </div>
            <div class="flex flex-col gap-5">
                <div class="w-10/12 mx-auto bg-gray-100 shadow-md rounded mb-5">
                   <div
    class="w-full grid grid-cols-10 items-center divide-x divide-[#f1f1f4] sticky -top-5 bg-gray-100 rounded-t">
        ...
        <span class="block w-24 lg:w-full">عملیات</span>
                    ...
                    <div class="bg-white divide-y divide-[#f1f1f4]">
                        @php
                            $i = 1;
                        @endphp
                       @foreach ($lessonWithAttachments as $lessonAttachment)
    <div class="w-full grid grid-cols-10 items-center divide-x divide-[#f1f1f4]">
        ...
        <div class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center col-span-2">
            <span class="block w-20 lg:w-full">{{ $lessonAttachment->title }}</span>
        </div>
        <div class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center col-span-2">
            <span class="block w-20 lg:w-full">{{ $lessonAttachment->price }}</span>
        </div>
        <div class="p-1 lg:p-3 text-xs lg:text-sm h-full flex items-center justify-center text-gray-900 text-center col-span-2">
            <span class="block w-20 lg:w-full">{{ $lessonAttachment->summary }}</span>
        </div>
        <div class="col-span-3 flex flex-row items-center justify-center gap-1">
    <a href="{{ route('LessonAttachment.single', [$lessonAttachment]) }}"
    ...
    <a href="{{ route('LessonAttachment.edit', [$lessonAttachment]) }}"
    ...
    <a href="{{ route('LessonAttachment.delete', [$lessonAttachment]) }}"
